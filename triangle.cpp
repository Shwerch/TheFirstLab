...
bool triangleCalculations(double sides[], double *perimeter, double *square,
                          bool *isosceles) {
  ...
  *square = pow(halfPerimeter * (halfPerimeter - sides[0]) *
...
int main() {
  while (true) {
    ...
    bool isosceles,
        result = triangleCalculations(sides, &perimeter, &square, &isosceles);
    if (result) {
      cout << "Стороны:  " << sides[0] << " " << sides[1] << " " << sides[2]
           << endl;
      cout << "Периметр: " << perimeter << endl;
      cout << "Площадь:  " << square << endl;
      cout << ((isosceles) ? "Равнобедренный" : "Не равнобедренный")
           << " треугольник" << endl;
    } else
